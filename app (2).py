...
def load_data():
    ...
    time_candidates = [col for col in df.columns if '時間' in col or '時' in col]
    if not time_candidates:
        st.error("❌ 無法辨識時間欄位，請確認資料中有包含「時間」或「時」的欄位名稱")
        st.stop()
    time_col = time_candidates[0]
    df[time_col] = pd.to_datetime(df[time_col], errors='coerce')
    df = df.dropna(subset=[time_col])
    df = df.rename(columns={time_col: '時間'})
    if '頻道名稱' not in df.columns or '在線人數' not in df.columns:
        st.error("❌ 資料格式錯誤，請確認含有『頻道名稱』與『在線人數』欄位")
        st.stop()
    df = df[df['頻道名稱'].notnull()]
    return df

st.title("📊 YouTube 直播頻道在線人數分析（雲端自動更新版）")
